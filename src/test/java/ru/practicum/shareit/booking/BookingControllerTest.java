...
class BookingControllerTest {
    ...
    @Test
    @DisplayName("POST create booking when end and start are in the past /bookings")
    ...
    @Test
    @DisplayName("GET booking of booker by all states /bookings")
    ...
    void getBookingsOfBookerByStateUnsupported() throws Exception {
        mockMvc.perform(get("/bookings?state=Unknown state")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header(userHeader, booker.getId()))
                .andExpect(status().isBadRequest())
                .andExpect(content().string("{\"error\":\"Unknown state: Unknown state\"}"));
    }
    ...
    @Test
    @DisplayName("GET booking of owner by all states /bookings/owner")
    ...
    void getBookingsOfOwnerByStateUnsupported() throws Exception {
        mockMvc.perform(get("/bookings/owner?state=Unknown state")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header(userHeader, owner.getId()))
                .andExpect(status().isBadRequest())
                .andExpect(content().string("{\"error\":\"Unknown state: Unknown state\"}"));
    ...
    @Test
    @DisplayName("GET booking when user is not an owner and not a booker /bookings")
